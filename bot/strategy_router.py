...
class StrategyRouterConfig:
    ...
    gold_strategy_config: str = "config/config.paper.gold-trend-runtime.json"
    gold_min_route_score: float = 0.0
    gold_takeover_advantage: float | None = None
    ...
    gold_state_db_path: str = "state/runtime_gold_usdt_router.db"
    gold_margin_mode: str = "isolated"
    gold_position_size_pct: float = 0.70
    gold_sizing_basis: str = "total_equity"
    gold_sizing_cash_buffer_usdt: float = 0.0
    gold_max_notional_usdt: float | None = None
    gold_min_order_notional_usdt: float = 10.0
    gold_min_rebalance_notional_usdt: float = 30000.0
    gold_min_rebalance_gap_ratio: float = 0.1
    gold_rebalance_cooldown_seconds: float = 14400.0
    gold_rebalance_on_leverage_change: bool = True
    gold_rebalance_on_notional_gap: bool = False
    gold_max_close_order_contracts: float | None = None
    gold_max_close_order_notional_usdt: float | None = None
    gold_max_market_order_contracts: float | None = None
    gold_market_order_chunk_delay_seconds: float = 0.0
    gold_close_confirm_timeout_seconds: float = 15.0
    gold_close_confirm_poll_seconds: float = 1.0
    gold_close_chunk_delay_seconds: float = 0.2
    gold_enable_exchange_stop: bool = True
    gold_rebalance_risk_on_market_hours_only: bool = False
...
class StrategyRouter:
    ...
    def _min_score_for(self, strategy_id: str) -> float:
        ...
        if strategy_id == "gold_usdt_trend":
            return float(self.config.gold_min_route_score)
        return 0.0
    ...
    def _takeover_advantage_for(self, challenger_strategy_id: str) -> float:
        ...
        if challenger_strategy_id == "gold_usdt_trend" and self.config.gold_takeover_advantage is not None:
            return float(self.config.gold_takeover_advantage)
    ...
    def _is_live_eligible(self, strategy_id: str) -> bool:
        ...
        if strategy_id == "gold_usdt_trend":
            return bool(self.config.gold_execution_enabled)
        return True
    ...
    def _collect_candidates(self) -> list[RoutedSignalCandidate]:
        ...
        from bot.gold_usdt_signal_adapter import GoldUsdtSignalAdapter
        ...
        if self.gold_adapter is None:
            self.gold_adapter = GoldUsdtSignalAdapter(Path(self.config.gold_strategy_config))
        ...
        if self.config.enable_gold:
            candidates.append(self.gold_adapter.preview())
        return candidates
