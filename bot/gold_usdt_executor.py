...
from scripts.replay_googl_usdt_4h import attach_gold_daily_state, load_okx_4h
...
class GoldUsdtExecutionEngine:
    def __init__(self, router_config: StrategyRouterConfig, gold_config_path: str | Path):
        self.router_config = router_config
        self.gold_config_path = Path(gold_config_path).resolve()
        self.gold_config = json.loads(self.gold_config_path.read_text())
        self.shadow_gate_profile = QqqShadowGateProfile.from_config(self.gold_config)
        ...
        self.store = StateStore(router_config.gold_state_db_path)
    ...
    def _load_markets_cache(self) -> dict[str, Any]:
        ...
        if not isinstance(payload, dict):
            return {}
        symbol = str(self.gold_config["execution_symbol"])
    ...
    def _exchange_leverage(self) -> float:
        configured = self.gold_config.get("exchange_leverage", self.gold_config.get("base_leverage", 10.0))
    ...
    def bootstrap(self) -> dict[str, Any]:
        ...
        try:
            ...
            if self.router_config.mode != "paper":
                ...
                self.client.set_leverage(leverage, symbol, margin_mode=self.router_config.gold_margin_mode, pos_side="long")
    ...
    def evaluate_latest(self, candidate: RoutedSignalCandidate | None) -> dict[str, Any]:
        ...
        if context is None:
            if exchange_position["contracts"] > 0:
                actions.append(self.close_position(reason="router_no_gold_signal"))
        ...
        if context.stop_hit:
            if exchange_position["contracts"] > 0:
                actions.append(
                    self.close_position(
                        reason="gold_trailing_stop_hit",
                        exit_price=context.stop_price,
                        timestamp=str(context.candidate.get("timestamp") or "runtime"),
                    )
                )
            else:
                actions.append({"status": "skipped", "reason": "gold_stop_hit_no_exchange_position"})
        ...
        if exchange_position["contracts"] <= 0:
            if not self._candidate_meets_entry_score(context):
                actions.append(
                    {
                        "status": "skipped",
                        "reason": "gold_entry_score_below_min",
                        "action": "open_gold_usdt_long",
                        "route_score": context.route_score,
                        "min_route_score": float(self.router_config.gold_min_route_score),
                    }
                )
            ...
            if not bool(risk_on_status["open"]):
                actions.append(
                    {
                        "status": "skipped",
                        "reason": "gold_risk_on_window_closed",
                        "action": "open_gold_usdt_long",
            ...
            if self._same_signal_stop_locked(context):
                actions.append(
                    {
                        "status": "skipped",
                        "reason": "gold_stop_hit_same_signal_lock",
                        "candidate_timestamp": context.candidate.get("timestamp"),
                    }
                )
                return {"status": "ok", "symbol": self.symbol, "actions": actions, "position_open": False}
            if not self._stop_price_valid_for_open(context):
                actions.append(
                    {
                        "status": "skipped",
                        "reason": "gold_invalid_initial_stop_price",
                        "action": "open_gold_usdt_long",
            ...
            if not bool(shadow_gate.get("allow", True)):
                actions.append(
                    {
                        "status": "skipped",
                        "reason": "gold_shadow_gate_blocked",
                        "action": "open_gold_usdt_long",
        ...
        leverage_rebalance = bool(self.router_config.gold_rebalance_on_leverage_change) and leverage_changed
        notional_rebalance = bool(self.router_config.gold_rebalance_on_notional_gap)
        ...
        if should_rebalance:
            ...
            actions.append(
                {
                    "status": "skipped",
                    "action": "rebalance_gold_position",
                    "reason": "gold_risk_on_window_closed",
    ...
    @property
    def symbol(self) -> str:
        return str(self.gold_config["execution_symbol"])

    def load_state(self) -> dict[str, Any]:
        raw = self.store.get_value("gold_usdt_state")
        if not raw:
            return {}
        try:
    ...
    def save_state(self, payload: dict[str, Any]) -> None:
        self.store.set_value("gold_usdt_state", json.dumps(payload, ensure_ascii=False, indent=2))

    def _load_shadow_gate_state(self) -> dict[str, Any]:
        raw = self.store.get_value("gold_shadow_gate_state")
        if not raw:
    ...
    def _save_shadow_gate_state(self, state: dict[str, Any]) -> None:
        self.store.set_value("gold_shadow_gate_state", json.dumps(self.shadow_gate.normalize_state(state), ensure_ascii=False, indent=2))
    ...
    def _append_shadow_gate_event(self, timestamp: str | None, event: dict[str, Any], state: dict[str, Any], decision: dict[str, Any] | None = None) -> None:
        ...
        self.store.append_action(str(timestamp or "runtime"), "GOLD_SHADOW_GATE", payload)

    def _shadow_gate_observation_timestamp(self, context: GoldOrderContext) -> str:
    ...
    def _shadow_gate_observe_context(self, context: GoldOrderContext) -> tuple[dict[str, Any], dict[str, Any] | None]:
    ...
    def _shadow_gate_pre_open(self, context: GoldOrderContext) -> dict[str, Any]:
    ...
    def _shadow_gate_record_open(self, context: GoldOrderContext) -> None:
        if not self.shadow_gate_profile.enabled:
            return
        state = self._load_shadow_gate_state()
        ...
        *,
        ...
        if not isinstance(state.get("position"), dict) and position:
            state["position"] = {
                ...
                "leverage": float(position.get("leverage", self.gold_config.get("base_leverage", 10.0)) or self.gold_config.get("base_leverage", 10.0)),
            }
        ...
        state, event = self.shadow_gate.record_close(
            ...
            reason=str(reason),
            taker_fee_rate=float(self.gold_config.get("taker_fee_rate", 0.0) or 0.0),
            slippage_bps=float(self.gold_config.get("slippage_bps", 0.0) or 0.0),
        )
        self._save_shadow_gate_state(state)
        if event is not None:
    ...
    def _signal_overrides(self) -> dict[str, Any]:
        overrides = self.gold_config.get("signal_overrides", {})
    ...
    def _load_gold_signal_path(self, signal_source: Path) -> pd.DataFrame:
        ...
        attach_gold_daily_state 需要 leverage_tier/target_leverage 列；旧信号缺列时
        ...
        if "leverage_tier" not in frame.columns:
            frame["leverage_tier"] = frame["position"].map({"GOLD": "base", "FLAT": "flat", "CASH": "flat"}).fillna("flat")
    ...
    def _closed_bars(self) -> pd.DataFrame:
        ...
        signal_source = self._resolve_path(str(self.gold_config["signal_source"]))
        signal_path = self._load_gold_signal_path(signal_source)
        bars = attach_gold_daily_state(self._load_bars(), signal_path, trim_to_signal_end=False)
        if bool(self.gold_config.get("use_closed_execution_bars", True)):
            bars = filter_closed_bars(
                bars,
                timeframe=str(self.gold_config.get("execution_timeframe", "4h")),
                grace_seconds=int(self.gold_config.get("closed_bar_grace_seconds", 30) or 0),
            )
        if bars.empty:
            raise RuntimeError("No closed GOLD/USDT bars available")
    ...
    def _latest_bars(self) -> tuple[pd.Series, float | None]:
        ...
        prev_close 供杠杆爬坡确认（无前视语义，与 run_gold_4h_replay 一致）：
    ...
    def _build_context(self, candidate: RoutedSignalCandidate) -> GoldOrderContext:
        ...
        base_leverage = float(self.gold_config["base_leverage"])
        full_leverage = float(candidate.leverage or base_leverage)
        stop_loss_pct = float(self.gold_config["stop_loss_pct"])
        ramp_confirm_pct = float(self.gold_config.get("ramp_confirm_pct", 0.0) or 0.0)
        ramp_pre_stop_pct = float(self.gold_config.get("ramp_pre_stop_pct", 0.0) or 0.0)
        state = self.load_state()
        ...
        return GoldOrderContext(
            symbol=self.symbol,
            margin_mode=str(self.router_config.gold_margin_mode),
    ...
    def _risk_on_window_status(self) -> dict[str, Any]:
        return market_time_window_status(
            enabled=bool(self.router_config.gold_rebalance_risk_on_market_hours_only),
            timezone_name=str(self.router_config.gold_market_hours_timezone),
            start_time=str(self.router_config.gold_market_hours_start),
            end_time=str(self.router_config.gold_market_hours_end),
            trading_calendar=str(self.router_config.gold_market_calendar),
        )

    def risk_on_window_status(self) -> dict[str, Any]:
        return self._risk_on_window_status()

    def _state_from_context(self, context: GoldOrderContext) -> dict[str, Any]:
        state = self.load_state()
        previous = state.get("position") if isinstance(state.get("position"), dict) else {}
    ...
    def _same_signal_stop_locked(self, context: GoldOrderContext) -> bool:
        state = self.load_state()
    ...
    def _candidate_meets_entry_score(self, context: GoldOrderContext) -> bool:
        return float(context.route_score) >= float(self.router_config.gold_min_route_score)

    @staticmethod
    def _stop_price_valid_for_open(context: GoldOrderContext) -> bool:
    ...
    def _sizing_capital_usdt(self) -> float:
        if self.router_config.mode == "paper":
            ...
            basis = str(self.router_config.gold_sizing_basis or "available").strip().lower()
    ...
    def _target_notional(self, context: GoldOrderContext) -> float:
        notional = self._sizing_capital_usdt() * float(self.router_config.gold_position_size_pct) * float(context.leverage)
        if self.router_config.gold_max_notional_usdt is not None:
            notional = min(notional, float(self.router_config.gold_max_notional_usdt))
        return max(notional, 0.0)

    def _rebalance_target_notional(
        self,
        context: GoldOrderContext,
        exchange_position: dict[str, Any],
        current: dict[str, Any],
    ) -> float:
        current_notional = float(exchange_position.get("notional_usdt", 0.0) or 0.0)
        current_leverage = float(current.get("leverage", 0.0) or 0.0)
        if bool(self.router_config.gold_rebalance_on_notional_gap):
            ...
            target = self._target_notional(context)
        if self.router_config.gold_max_notional_usdt is not None:
            target = min(target, float(self.router_config.gold_max_notional_usdt))
    ...
    def _rebalance_gap_large(self, notional_gap: float, current_notional: float, target_notional: float) -> bool:
        min_abs = max(0.0, float(self.router_config.gold_min_rebalance_notional_usdt or 0.0))
        if float(notional_gap) < min_abs:
            return False
        min_ratio = max(0.0, float(self.router_config.gold_min_rebalance_gap_ratio or 0.0))
        if min_ratio <= 0:
            return True
    ...
    def _rebalance_cooldown_status(self, current: dict[str, Any]) -> dict[str, Any]:
        cooldown_seconds = max(0.0, float(self.router_config.gold_rebalance_cooldown_seconds or 0.0))
    ...
    def _build_context_from_state(self, position: dict[str, Any]) -> GoldOrderContext:
        return GoldOrderContext(
            symbol=self.symbol,
            margin_mode=str(self.router_config.gold_margin_mode),
            leverage=float(position.get("leverage", self.gold_config["base_leverage"]) or self.gold_config["base_leverage"]),
            stop_loss_pct=float(position.get("stop_loss_pct", self.gold_config["stop_loss_pct"]) or self.gold_config["stop_loss_pct"]),
    ...
    def open_position(self, context: GoldOrderContext) -> dict[str, Any]:
        if not self._stop_price_valid_for_open(context):
            return {
                "status": "skipped",
                "reason": "gold_invalid_initial_stop_price",
        ...
        if notional < float(self.router_config.gold_min_order_notional_usdt):
        ...
        self.store.append_action(str(context.candidate.get("timestamp") or "runtime"), "OPEN_GOLD_USDT", payload)
        return payload
    ...
    def restore_position(self, rollback: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        """
        if self.router_config.mode == "paper":
            return {"status": "paper_restored", "symbol": self.symbol, "action": "restore_gold_usdt_long"}
        ...
        stop_loss_pct = float(self.gold_config.get("stop_loss_pct", 4.0) or 4.0)
        ...
        leverage = float(
            position_state.get("leverage", self.gold_config["base_leverage"])
            if isinstance(position_state, dict)
            else self.gold_config["base_leverage"]
        ) or float(self.gold_config["base_leverage"])
        exchange_leverage = self._exchange_leverage()
        ...
        if exchange_stops:
            payload["exchange_stops"] = exchange_stops
        self.store.append_action("runtime", "RESTORE_GOLD_USDT", payload)
        return payload

    def sync_leverage_setting(self, context: GoldOrderContext) -> dict[str, Any]:
        exchange_leverage = self._exchange_leverage()
        if self.router_config.mode == "paper":
            payload = {
                "status": "paper_synced",
                "action": "sync_gold_leverage_setting",
                "leverage": exchange_leverage,
                "exchange_leverage": exchange_leverage,
                "target_exposure_leverage": context.leverage,
            }
            self.store.append_action(str(context.candidate.get("timestamp") or "runtime"), "SYNC_GOLD_LEVERAGE", payload)
            return payload
        try:
            ...
            self.store.append_action(str(context.candidate.get("timestamp") or "runtime"), "SYNC_GOLD_LEVERAGE_FAILED", payload)
            return payload
        payload = {
            "status": "submitted",
            "action": "sync_gold_leverage_setting",
    ...
    def rebalance_position(
        ...
        *,
        ...
        min_gap = float(self.router_config.gold_min_rebalance_notional_usdt)
        ...
        if self.router_config.mode == "paper":
            ...
            self.store.append_action(str(context.candidate.get("timestamp") or "runtime"), "REBALANCE_GOLD_POSITION", payload)
            return payload
        ...
        if side == "buy":
            ...
            if leverage_result.get("status") == "error":
                ...
                self.store.append_action(str(context.candidate.get("timestamp") or "runtime"), "REBALANCE_GOLD_POSITION_FAILED", payload)
                return payload
            amount = self._order_amount(abs(delta_notional), context.reference_price)
            if amount <= 0:
                return {"status": "skipped", "action": "rebalance_gold_position", "reason": "non_positive_add_amount"}
            chunks = self._market_order_chunks(amount)
            orders = []
            exchange_stops = []
            for idx, chunk_amount in enumerate(chunks):
        ...
        if amount <= 0:
            return {"status": "skipped", "action": "rebalance_gold_position", "reason": "non_positive_reduce_amount"}
    ...
    def close_position(self, *, reason: str, exit_price: float | None = None, timestamp: str | None = None) -> dict[str, Any]:
        ...
        if amount <= 0:
            return {"status": "skipped", "reason": "no_open_gold_position"}
        if self.router_config.mode == "paper":
        ...
        for idx, chunk_amount in enumerate(chunks):
            order = self.client.create_order(
                ...
                "sell",
                chunk_amount,
                params={"reduceOnly": True, "tdMode": self.router_config.gold_margin_mode, "posSide": "long"},
            )
            orders.append({"order": order, "amount": chunk_amount})
            chunk_delay = self._sell_chunk_delay_seconds()
            if idx < len(chunks) - 1 and chunk_delay > 0:
        ...
        self.store.append_action("runtime", "CLOSE_GOLD_USDT", payload)
    ...
    def sync_external_flat(
        ...
        *,
        reason: str,
        context: GoldOrderContext | None = None,
        state: dict[str, Any] | None = None,
    ) -> dict[str, Any]:
        state = state if isinstance(state, dict) else self.load_state()
        position_state = state.get("position") if isinstance(state.get("position"), dict) else None
        if position_state is None:
            return {"status": "skipped", "reason": "no_local_gold_position", "action": "sync_external_flat"}
        exit_price = None
        timestamp = None
        ...
        close_reason = "gold_external_stop_sync" if stop_like else str(reason)
        self._shadow_gate_record_close(
            position_state,
            reason=close_reason,
    ...
    def _close_order_chunks(self, amount: float, notional_usdt: float) -> list[float]:
        ...
        if self.router_config.gold_max_close_order_contracts is not None:
            chunk_limits.append(float(self.router_config.gold_max_close_order_contracts))
        if self.router_config.gold_max_close_order_notional_usdt is not None and notional_usdt > 0:
            chunk_limits.append(float(amount) * float(self.router_config.gold_max_close_order_notional_usdt) / float(notional_usdt))
    ...
    def _market_order_chunk_limits(self) -> list[float]:
        chunk_limits: list[float] = []
        if self.router_config.gold_max_market_order_contracts is not None:
            chunk_limits.append(float(self.router_config.gold_max_market_order_contracts))
        try:
            market = self._market()
    ...
    def _market_order_chunk_delay_seconds(self) -> float:
        configured = self.router_config.gold_market_order_chunk_delay_seconds
        if configured is not None:
            return max(0.0, float(configured))
        return max(0.0, float(self.router_config.gold_close_chunk_delay_seconds))
    ...
    def _wait_until_flat(self) -> dict[str, float]:
        deadline = time.time() + max(0.0, float(self.router_config.gold_close_confirm_timeout_seconds))
        poll = max(0.1, float(self.router_config.gold_close_confirm_poll_seconds))
    ...
    def _attach_stop_to_order_params(self, params: dict[str, Any], context: GoldOrderContext) -> dict[str, Any] | None:
        if not bool(self.router_config.gold_enable_exchange_stop) or context.stop_price <= 0:
            return None
        market = self._market()
    ...
    def update_trailing_stop(self, context: GoldOrderContext, exchange_position: dict[str, Any]) -> dict[str, Any] | None:
        state = self.load_state()
        current = state.get("position") if isinstance(state.get("position"), dict) else {}
        ...
        if self.router_config.mode != "paper" and bool(self.router_config.gold_enable_exchange_stop):
            ...
            if exchange_stop is not None and exchange_stop.get("status") == "error":
                payload = {
                    "status": "error",
                    "action": "update_gold_trailing_stop",
                ...
                self.store.append_action(str(context.candidate.get("timestamp") or "runtime"), "UPDATE_GOLD_STOP_FAILED", payload)
                return payload
        ...
        self.store.append_action(str(context.candidate.get("timestamp") or "runtime"), "UPDATE_GOLD_STOP", payload)
        return payload
    ...
    def _generate_attach_algo_client_id(self) -> str:
        return f"golds{uuid.uuid4().hex[:26]}"
    ...
    def _refresh_exchange_stop_identity(
        ...
        *,
        ...
        result: dict[str, Any] = {
            "status": "disabled" if not bool(self.router_config.gold_enable_exchange_stop) else "not_found",
            "order_id": order_id,
            "algo_id": None,
            "algo_client_id": attach_algo_client_id,
        }
        if self.router_config.mode == "paper" or not bool(self.router_config.gold_enable_exchange_stop):
            return result
        try:
            position = self.fetch_position_state()
