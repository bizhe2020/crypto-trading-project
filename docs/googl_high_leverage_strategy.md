...
- **maxDD 69% 的真实含义**：0.75x 档 + 杠杆爬坡（§5.4）收益 +630% 但 maxDD 69%（回撤期间需 +222% 才回本，优于无爬坡的 79%/需 +376%）。每次 4% 止损 × 11.2x ≈ 45% 权益回撤，连吃两次即 80%——爬坡把"入场即被止损"的两笔大亏（-42%/-39%）压到 base 7.5x（-29%/-28%），是 maxDD 下降主因。这是"用 maxDD 换 CAGR"的本质，实盘需强回撤承受力或手动降档。
...
- **20x 上限 vs 可行性**：合约支持 20x，但真实数据回测（§5.2）表明 15x/10x 被严格支配（+397%/maxDD 90%），0.75x 乘数（11.2x/7.5x）+ 杠杆爬坡（§5.4）是收益最优点（+630%/maxDD 69%）；1.25x 以上崩溃。**已按 11.2x/7.5x + ramp 0.5% 定档，maxDD 69% 是已知代价**。
