...
= **+2319.64% / maxDD 50.49%**，17 笔 / 47% 胜）。注入点：`run_googl_4h_replay`
新增 `lev_scale_col`（per-bar 杠杆缩放）与 `stop_pct_col`（per-bar 止损宽度），
默认 None 不改变行为（回归测试 24 绿）。研究脚本：`run_googl_vol_target_ab.py`、
`run_googl_earnings_derisk_ab.py`、`run_googl_adaptive_stop_ab.py`。
...
|---|---|---|---|
| 基线 0.75x | — | +2319.64% | 50.49% |
| 0.5/1.5（低波动加杠杆） | 0.994 | +1190.06% | 56.35% |
| 0.5/1.0（只高波动降杠杆） | 0.882 | +1377.27% | 49.32% |

低波动加杠杆反而把 maxDD 顶到 56%（平静后爆波动的"波动率悬崖"）；只降不加只把
maxDD 压 1.2pp 却砍掉 41% 收益。**波动率缩放与 4% trailing stop + shadow gate
...
|---|---|---|
| 基线 4% | +2319.64% | 50.49% |
| 0.75/1.5 | +2449.66% | 49.33% |
| 0.75/1.25 | +1957.70% | 57.76% |
| 1.0/1.5（只放宽不收紧） | +2176.38% | 50.49% |

hi（高波动放宽上限）是稳健平台（1.5→3.0 结果不变）；lo（低波动收紧下限）是悬崖
（0.70→+1488%、0.75→+2449%、0.80→+2417%）。改进只落在信念前段（+263→+303%），
信念后段反而变差（+566→+532%、DD 45.5→48.3）。**lo 悬崖 + regime 依赖 = 单笔交易
翻转的过拟合信号，不采纳**（与 §5.5 的 pre_stop 刃口同款教训）。
