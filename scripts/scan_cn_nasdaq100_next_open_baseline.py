...
DEFAULT_OUTPUT = ROOT / "var" / "reports" / "cn_nasdaq100_next_open_baseline_scan.json"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Scan the formal next-open baseline for CN Nasdaq-100 ETF.")
    parser.add_argument("--config", default=str(ROOT / "config" / "config.paper.cn-nasdaq100-etf.json"))
    parser.add_argument("--output", default=str(DEFAULT_OUTPUT))
    parser.add_argument("--entry-fast-values", default="20,25,30")
    parser.add_argument("--entry-slow-values", default="150,180,200,220")
...
def build_tiered_rules(vix_low_weak: float, vix_normal_strong: float) -> list[dict[str, Any]]:
    return [
        ...
        {
            "vix_label": "vix_low",
            "leverage": float(vix_low_weak),
        },
        {
            "vix_label": "vix_normal",
            "rel_strength_label": "qqq_strong",
            "leverage": float(vix_normal_strong),
        },
    ]
...
def main() -> None:
                    ...
                    for drawdown in parse_floats(args.trailing_drawdown_values):
                        for vix_low_weak in parse_floats(args.vix_low_weak_values):
                            for vix_normal_strong in parse_floats(args.vix_normal_strong_values):
                                ...
                                config["entry_fast_window"] = fast
                                config["entry_slow_window"] = slow
                                config["max_hold_days"] = max_hold
                                config["trailing_lookback_days"] = lookback
    ...
    for item in ranked[:10]:
        s = item["summary"]
        print(
            item["entry_fast_window"],
            item["entry_slow_window"],
            item["max_hold_days"],
