...
DEFAULT_OUTPUT = ROOT / "var" / "reports" / "qqq_usdt_stop_leverage_grid.json"

STOP_VALUES = [1.0, 1.5, 2.0, 2.5]
LEVERAGE_PROFILES = {
    "fixed6": {"base": 6.0, "offense": 6.0, "defense": 6.0},
    "fixed8": {"base": 8.0, "offense": 8.0, "defense": 8.0},
    "fixed10": {"base": 10.0, "offense": 10.0, "defense": 10.0},
    "dyn_cap6": {"base": 4.0, "offense": 6.0, "defense": 2.0},
    "dyn_cap8": {"base": 6.0, "offense": 8.0, "defense": 2.0},
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Grid scan stop-loss and leverage caps for QQQ/USDT replay.")
    ...
    by_return = sorted(results, key=lambda x: (x["summary"]["total_return_pct"], -x["summary"]["max_drawdown_pct"]), reverse=True)
    ...
    payload = {
        ...
        "top_by_return": by_return[:8],
        "top_by_score": by_score[:8],
        "results": results,
    }
    ...
    print(out)
    print(json.dumps({"top_by_return": by_return[:8], "top_by_score": by_score[:8]}, ensure_ascii=False, indent=2))
