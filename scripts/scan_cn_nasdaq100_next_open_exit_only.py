...
DEFAULT_OUTPUT = ROOT / "var" / "reports" / "cn_nasdaq100_next_open_exit_only_scan.json"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Exit-only scan for the formal next-open CN Nasdaq-100 ETF baseline.")
...
def main() -> None:
    ...
    payload = {
        "current_formal_baseline": {
            "max_hold_days": int(base_config.get("max_hold_days", 90)),
            "trailing_lookback_days": int(base_config.get("trailing_lookback_days", 10)),
            "trailing_drawdown_pct": float(base_config.get("trailing_drawdown_pct", 5.0)),
    ...
    for item in ranked[:10]:
        s = item["summary"]
        print(item["max_hold_days"], item["trailing_lookback_days"], item["trailing_drawdown_pct"], s["total_return_pct"], s["max_drawdown_pct"], s["yearly_returns_pct"])
